...
int OnInit() {
   ...
   IndicatorSetString(INDICATOR_LEVELTEXT,0, "range");
   SetIndexBuffer(0, range);
   
   IndicatorSetString(1, "upup (1)");
   SetIndexBuffer(1, upup);
   IndicatorSetString(INDICATOR_LEVELTEXT,2, "updn (2)");
   SetIndexBuffer(2, updn);
   IndicatorSetString(INDICATOR_LEVELTEXT,3, "dndn (3)");
   SetIndexBuffer(3, dndn);
   IndicatorSetString(INDICATOR_LEVELTEXT,4, "dnup (4)");
   SetIndexBuffer(4, dnup);
   
   IndicatorSetString(INDICATOR_LEVELTEXT,5, "noSqz (5)");
   SetIndexBuffer(5, no);
   PlotIndexSetInteger(5,PLOT_ARROW,167);
   IndicatorSetString(INDICATOR_LEVELTEXT,6, "sqzOn (6)");
   SetIndexBuffer(6, On);
   PlotIndexSetInteger(6,PLOT_ARROW,167);
   IndicatorSetString(INDICATOR_LEVELTEXT,7, "sqzOff (7)");
   SetIndexBuffer(7, Off);
   PlotIndexSetInteger(7,PLOT_ARROW,167);
   
   IndicatorSetString(INDICATOR_LEVELTEXT,8, "linregsrc");
   SetIndexBuffer(8, linregsrc);
   IndicatorSetString(INDICATOR_LEVELTEXT,9, "linreg");
...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[],
   ...
   for(int pos=startPos; pos>=0; pos--) {
      // Calculate BB
      double basis   = iMA(_Symbol, PERIOD_CURRENT, length, 0, MODE_SMA, PRICE_CLOSE);
      double dev     = mult * iStdDev(_Symbol,PERIOD_CURRENT,length,0,MODE_SMA,PRICE_CLOSE);
      double upperBB = basis + dev;
      double lowerBB = basis - dev;
      
      // Calculate KC
      double ma      = iMA(_Symbol, PERIOD_CURRENT, lengthKC, 0, MODE_SMA, PRICE_CLOSE);
      range[pos]     = useTrueRange ? tr(pos) : (high[pos] - low[pos]);
      double rangema = iMA(range[pos], 0, lengthKC, 0, MODE_SMA, pos);
